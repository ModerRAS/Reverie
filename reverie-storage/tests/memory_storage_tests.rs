...
use reverie_storage::{
    TrackStorage, AlbumStorage, ArtistStorage, UserStorage, PlaylistStorage, FileStorage, Storage
};
use chrono::Utc;
use uuid::Uuid;
...
async fn test_memory_storage_track_operations() {
    ...
    // Test list tracks
    let tracks = storage.list_tracks(10, 0).await.expect("Failed to list tracks");
    assert_eq!(tracks.len(), 1);

    // Test search tracks
    let search_results = storage.search_tracks("Test").await.expect("Failed to search tracks");
    assert_eq!(search_results.len(), 1);

    // Test delete track
    storage.delete_track(track.id).await.expect("Failed to delete track");
    let deleted = storage.get_track(track.id).await.expect("Failed to get deleted track");
...
async fn test_memory_storage_album_operations() {
    ...
    let retrieved = storage.get_album(album.id).await.expect("Failed to get album");
    assert!(retrieved.is_some());
    assert_eq!(retrieved.unwrap().name, album.name);

    // List albums
    let albums = storage.list_albums(10, 0).await.expect("Failed to list albums");
    assert_eq!(albums.len(), 1);

    // Delete album
    storage.delete_album(album.id).await.expect("Failed to delete album");
    let deleted = storage.get_album(album.id).await.expect("Failed to get deleted album");
    assert!(deleted.is_none());
}
...
async fn test_memory_storage_artist_operations() {
    ...
    let retrieved = storage.get_artist(artist.id).await.expect("Failed to get artist");
    assert!(retrieved.is_some());
...
async fn test_memory_storage_playlist_operations() {
    ...
    // Get playlist
    let retrieved = storage.get_playlist(playlist.id).await.expect("Failed to get playlist");
    assert!(retrieved.is_some());
    
    // Get playlists by user
    let user_playlists = storage.get_playlists_by_user(user_id).await.expect("Failed to get user playlists");
    ...
    storage.add_track_to_playlist(&playlist_track).await.expect("Failed to add track to playlist");

    // Get playlist tracks
    let tracks = storage.get_playlist_tracks(playlist.id).await.expect("Failed to get playlist tracks");
    assert_eq!(tracks.len(), 1);

    // Remove track from playlist
    storage.remove_track_from_playlist(playlist.id, track_id).await.expect("Failed to remove track from playlist");
    let tracks = storage.get_playlist_tracks(playlist.id).await.expect("Failed to get playlist tracks after removal");
...
async fn test_memory_storage_file_operations() {
    ...
    // Write file
    storage.write_file(file_path, file_data).await.expect("Failed to write file");

    // Check file exists
    let exists = storage.file_exists(file_path).await.expect("Failed to check file exists");
    assert!(exists);

    // Read file
    let read_data = storage.read_file(file_path).await.expect("Failed to read file");
    assert_eq!(read_data, file_data);

    // Get file metadata
    let metadata = storage.get_file_metadata(file_path).await.expect("Failed to get file metadata");
    assert_eq!(metadata.size, file_data.len() as u64);
    assert!(metadata.is_file);

    // List files
    let files = storage.list_files("/test").await.expect("Failed to list files");
    assert!(files.contains(&file_path.to_string()));

    // Delete file
    storage.delete_file(file_path).await.expect("Failed to delete file");
    let exists = storage.file_exists(file_path).await.expect("Failed to check file exists after deletion");
    assert!(!exists);
}
...
async fn test_memory_storage_health_check() {
    ...
    let healthy = storage.health_check().await.expect("Failed to check health");
    assert!(healthy);
...
async fn test_memory_storage_relationships() {
    ...
    let artist_albums = storage.get_albums_by_artist(artist.id).await.expect("Failed to get artist albums");
    assert_eq!(artist_albums.len(), 1);

    // Test get tracks by album
    let album_tracks = storage.get_tracks_by_album(album.id).await.expect("Failed to get album tracks");
    assert_eq!(album_tracks.len(), 3);

    // Test get tracks by artist
    let artist_tracks = storage.get_tracks_by_artist(artist.id).await.expect("Failed to get artist tracks");
