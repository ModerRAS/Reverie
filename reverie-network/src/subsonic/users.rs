...
/// GET /rest/updateUser - 更新用户
pub async fn update_user_handler<S: SubsonicStorage + Clone>(
    State(state): State<SubsonicState<S>>,
    Query(params): Query<HashMap<String, String>>,
) -> Response {
    let username = match params.get("username") {
    ...
    let jukebox_role = params.get("jukeboxRole").and_then(|s| s.parse().ok());
    let download_role = params.get("downloadRole").and_then(|s| s.parse().ok());
    let upload_role = params.get("uploadRole").and_then(|s| s.parse().ok());
    let playlist_role = params.get("playlistRole").and_then(|s| s.parse().ok());
    let cover_art_role = params.get("coverArtRole").and_then(|s| s.parse().ok());
